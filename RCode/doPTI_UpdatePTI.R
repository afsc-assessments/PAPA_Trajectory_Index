...
dirCurr = file.path(dirPrj,"ResultsCurrent");
if (!dir.exists(dirCurr)) {
  dir.create(dirCurr);
} else {
  if (COPY_CURR){
    #--copy "current" files to ResultsPrevious
    fns = list.files(dirCurr,all.files=FALSE,full.names=FALSE);
    dirPrev = file.path(dirPrj,"ResultsPrevious");
    if (!dir.exists(dirPrev)) dir.create(dirPrev);
    res = file.copy(from=file.path(dirCurr,fns),to=file.path(dirPrev,fns),
                    overwrite=TRUE,copy.mode=TRUE,copy.date=TRUE);
    if (any(!res)){
      for (i in 1:length(res)) {
        if (!res[i]){
          str = paste0("File not copied from ResultsCurrent: ",fns[i]);
          warning(str);
        }
      }
      stop("Stopping. Check problematic files that didn't get copied.")
    }
    file.remove(file.path(dirCurr,fns));
  }#--if NEW_YEAR
}

fnBase="PTI_";
path=file.path(dirPrj,"Trajectories");

#--run the OSCURS model
#----first valid year for website data is Dec 1, 1967.
if (NEW_YEAR){
  #--run the model for YEAR and save output to {dirPrj}/Trajectories
  randNum<-round(runif(1,1,10000));
  res1<-rOSCURS::runOSCURS(fnBase=fnBase,
                           path=path,
...
if (NEW_YEAR){
  ...
  lst1<-rOSCURS::convertOSCURStoTbl(fnBase=file.path(path,fnBase),
  ...
  dfrPTI = readr::read_csv(file=file.path("./RCode","PTI_EndLocations1902-1967.csv"),show_col_types = FALSE);
  dfrPTI = dplyr::bind_rows(dfrPTI,data.frame(year=as.numeric(format(as.Date(tmp$dayEnd),"%Y")),lat=tmp$latEnd,lon=tmp$lonEnd)) |>
             dplyr::mutate(diff_prev=lat-Hmisc::Lag(lat,shift=1));
  trks   = lst1$tracks;
...
p <- ggplot(dfrPTI,mapping=aes(x=year,y=lat,colour="PTI")) +
      geom_line(mapping=aes(y=mn_lat,colour="L-T-M"),linewidth=1.25) + #--long-term mean 
...
#      geom_smooth(method="loess",color="yellow") +
      geom_line(mapping=aes(y=krn_lat,colour="5-Yr Running"),linewidth=1.25,alpha=0.8) + #--running average PTI
